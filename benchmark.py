...
# gcloud compute --project "hca-scale" ssh --zone "us-east1-b" "ll-knn"
#
...
def pynndescent_regular(X, threads=1, n_neighbors=25, max_candidates=50):
    ...
    t0 = time.time()
    index = NNDescent(X, n_neighbors=n_neighbors, max_candidates=max_candidates)
...
def pynndescent_threaded(X, threads=1, n_neighbors=25, max_candidates=50):
    t0 = time.time()
    index = NNDescent(X, n_neighbors=n_neighbors, max_candidates=max_candidates, n_jobs=threads)
    indices, distances = index._neighbor_graph
    t1 = time.time()
    return indices, distances, t1-t0
