...
def test_init_current_graph():
    ...
    current_graph_dask = pynndescent.dask.init_current_graph(
        data, data.shape, dist, dist_args, n_neighbors, chunk_size=4
    )
...
def test_new_build_candidates():
    ...
    candidate_neighbors_combined_dask = pynndescent.dask.new_build_candidates(
        current_graph_dask,
        n_vertices,
        n_neighbors,
        max_candidates,
