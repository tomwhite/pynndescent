...
from pynndescent.spark import get_chunk_sizes, apply_heap_updates_jit, apply_new_and_old_heap_updates_jit
...
def make_heap_bag(n_points, size, chunk_size):
    ...
    return db.from_sequence(chunk_sizes, npartitions=len(chunk_sizes)).map(make_heap_chunk)
...
def init_current_graph_bag(
        data, data_shape, dist, dist_args, n_neighbors, chunk_size
):
...
def nn_descent(
    ...
    current_graph_bag = init_current_graph_bag(
    ...
    for n in range(n_iters):
        ...
        candidate_neighbors_combined = build_candidates_bag(
            current_graph_bag,
            n_vertices,
            n_neighbors,
            max_candidates,
        ...
        updates_bag = group_by_key(
            map_partitions_with_index(candidate_neighbors_combined, listify(create_heap_updates))) \
            .map(lambda pair: pair[1])
